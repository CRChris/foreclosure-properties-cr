...
export function OpportunityMatrixCard({ auction }: OpportunityMatrixCardProps) {
  ...
  const securityBannerClass = {
    tier_1: 'bg-emerald-50 text-emerald-900 border-emerald-300 dark:bg-emerald-950/60 dark:border-emerald-500/50 dark:text-emerald-200',
    tier_2: 'bg-amber-50 text-amber-900 border-amber-300 dark:bg-amber-950/60 dark:border-amber-500/50 dark:text-amber-200',
    tier_3: 'bg-rose-50 text-rose-900 border-rose-300 dark:bg-rose-950/60 dark:border-rose-500/50 dark:text-rose-200',
  }[security.tier];

  return (
    <>
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-7 shadow-md dark:shadow-xl space-y-6 relative overflow-hidden">
        ...
        {/* Header Section */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-slate-100 dark:border-slate-800 relative z-10">
          <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 relative z-10">
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          <div className="bg-slate-50 dark:bg-slate-950/80 p-3.5 rounded-2xl border border-slate-200 dark:border-slate-800/80 space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="text-slate-700 dark:text-slate-300 font-semibold">{t.matrix.discountAdvantage}</span>
          ...
          <div className="bg-slate-50 dark:bg-slate-950/80 p-3.5 rounded-2xl border border-slate-200 dark:border-slate-800/80 space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="text-slate-700 dark:text-slate-300 font-semibold">{t.matrix.valuationSpread}</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="bg-slate-50 dark:bg-slate-950/80 p-3 rounded-2xl border border-slate-200 dark:border-slate-800/80">
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-700 dark:text-slate-300 font-semibold">{t.matrix.assetLiquidity}</span>
                <span className="font-mono font-bold text-sky-600 dark:text-sky-400">+{alpha.assetPoints}/10</span>
              </div>
            </div>

            <div className="bg-slate-50 dark:bg-slate-950/80 p-3 rounded-2xl border border-slate-200 dark:border-slate-800/80">
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-700 dark:text-slate-300 font-semibold">{t.matrix.roadFrontageFactor}</span>
                <span className="font-mono font-bold text-emerald-600 dark:text-emerald-400">+{alpha.accessPoints}/5</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 flex flex-col justify-between space-y-4">
          ...
          <div className={`p-4 rounded-2xl border ${securityBannerClass} space-y-2`}>
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="font-mono text-xs font-bold px-2 py-0.5 rounded bg-white/40 dark:bg-black/40 border border-slate-300 dark:border-white/10">
                {security.score}/100
              </span>
            </div>
          ...
          <div className="bg-slate-50 dark:bg-slate-950/90 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-2">
      ...
      <div className="p-4 rounded-2xl bg-gradient-to-r from-emerald-50 via-slate-50 to-teal-50 dark:from-emerald-950/70 dark:via-slate-950 dark:to-teal-950/70 border border-emerald-200 dark:border-emerald-500/30 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-2.5">
          <FileCheck2 className="w-5 h-5 text-emerald-600 dark:text-emerald-400 shrink-0" />
          <div>
            <p className="font-bold text-slate-900 dark:text-slate-200">{t.matrix.statutoryProtection}</p>
        ...
        <span className="px-3 py-1.5 rounded-xl bg-emerald-100 text-emerald-800 dark:bg-emerald-500/10 border border-emerald-300 dark:border-emerald-500/40 dark:text-emerald-300 font-mono font-bold whitespace-nowrap shrink-0 text-[11px]">
